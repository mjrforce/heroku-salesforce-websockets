...
var socket = io.sockets.on('connection', async function (socket) {
    ...
    let payload = { id: recordId, username: username, userid: userid, count: membersarray.length, members: membersarray };
 ...
 async function disconnectDuplicateSockets(uid){
  let sockets = await io.in(uid).fetchSockets();
  for(const s of sockets){
    s.disconnect();
  }
 }
